-- Enable pg_trgm for trigram text matching
create extension if not exists pg_trgm;

-- RPC that searches recipes by title, description, OR ingredient name across the full
-- recipe collection. Replaces the client-side ingredient fallback that only checked
-- the 12 already-paginated records, silently missing matches elsewhere.
...
grant execute on function search_recipes_by_ingredient(text, uuid, text, text[], text, int, int)
  to authenticated;
