...
- **Routing via `wouter`.** [src/App.tsx](src/App.tsx) imports `Route, Switch, Redirect, useLocation` from `wouter` and defines real routes — `/`, `/recipes` (nest), `/community` (nest), `/planner` (nest), `/chat`, `/settings`, and `/admin` (admin-gated). Top-level panel switching is route-based; modals (recipe form, meal form, detail views, import/photo/onboarding) are still driven by App-level/page-level boolean state layered on top of the routes. PostHog pageviews are tracked per route.
